...
type ConstructorParametersList<TBases extends Constructor[]> = {
    [K in keyof TBases]: TBases[K] extends Constructor<infer U> ? ConstructorParameters<TBases[K]> : never;
};
...
export default function Mixin<TBases extends Constructor[]>(...bases: TBases) {
    ...
    bases.forEach(Base => {
        ...
        Object.getOwnPropertyNames(Base).forEach(name => {
            // console.warn(name)
            if (name !== 'prototype' && name !== 'name' && name !== 'length' && name !== "__eventHandlers") {
        ...
        Object.getOwnPropertyNames(Base.prototype).forEach(name => {
            ...
            if (name !== 'constructor' && name !== "__eventHandlers" && name !== "onRunJob") {
    ...
    type StaticProps = UnionToIntersection<StaticProperties<typeof bases[number]>>;
